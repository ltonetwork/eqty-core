...
export default class AnchorClient {
    ...
    async anchorMany(anchors) {
        try {
            ...
            if (fee > 0n && eqtyTokenAddress !== ZERO_ADDRESS) {
                ...
                const eqtyToken = new Contract(eqtyTokenAddress, [
    ...
    async hasSufficientBalance(anchorCount) {
        ...
        const userAddress = await this.signer.getAddress();
        const eqtyToken = new Contract(tokenAddress, ["function balanceOf(address account) external view returns (uint256)"], this.signer);
        const balance = await eqtyToken.balanceOf(userAddress);
    ...
    async hasSufficientAllowance(anchorCount) {
        ...
        const totalFee = await this.getTotalFee(anchorCount);
        const userAddress = await this.signer.getAddress();
        const eqtyToken = new Contract(tokenAddress, [
            "function allowance(address owner, address spender) external view returns (uint256)",
        ], this.signer);
        const allowance = await eqtyToken.allowance(userAddress, this.contract.target);
