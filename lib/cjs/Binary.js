...
class Binary extends Uint8Array {
    ...
    hash() {
        ...
        return Binary.fromHex(hashHex.slice(constants_1.HEX_PREFIX_LENGTH)); // Remove '0x' prefix
    }
    ...
    static fromHex(value) {
        try {
            const hex = value.startsWith(constants_1.HEX_PREFIX)
                ? value.slice(constants_1.HEX_PREFIX_LENGTH)
                : value;
            if (hex.length % constants_1.BYTES_PER_HEX_CHAR !== 0) {
            ...
            for (let i = 0; i < hex.length; i += constants_1.BYTES_PER_HEX_CHAR) {
                const byte = parseInt(hex.substr(i, constants_1.BYTES_PER_HEX_CHAR), 16);
                if (isNaN(byte)) {
                    throw new Error(`Invalid hex character at position ${i}`);
                }
                bytes[i / constants_1.BYTES_PER_HEX_CHAR] = byte;
            }
