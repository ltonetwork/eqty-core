...
class EventChain {
     ...
     */
    getIncrementalAnchorMap() {
        if (this.events.length === 0) {
            return [{ key: this.stateHash, value: this.stateHash }];
        }
        const anchors = [];
        let currentState = new Binary_1.default((0, ethers_1.keccak256)(this.id));
        for (const event of this.events) {
            const eventHash = event.hash;
            const combined = Binary_1.default.concat(currentState, eventHash);
            const newState = new Binary_1.default((0, ethers_1.keccak256)(combined));
            anchors.push({
                key: currentState,
                value: eventHash,
            });
